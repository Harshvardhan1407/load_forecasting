...
def get_current_system_confgiuration():
    try:
        virtual_memory = psutil.virtual_memory()
        # Total memory (in GB)
        total_memory = round(virtual_memory.total / (1024 ** 2),2)
        # Available memory (in GB)
        available_memory = round(virtual_memory.available / (1024 ** 2),2)
        # Used memory (in GB)
        used_memory = round( virtual_memory.used / (1024 ** 2),2)
        # Memory usage percentage
        memory_percentage = virtual_memory.percent
        logger.info(f"Total RAM: {total_memory:.2f} MB")
        logger.info(f"Available RAM: {available_memory:.2f} MB")
        logger.info(f"Used RAM: {used_memory:.2f} MB")
        logger.info(f"Memory Usage: {memory_percentage}%")
        return available_memory,total_memory
    except Exception as e:
        logger.error(f"error in get_current_system_confgiuration: {e}")

def calculate_collection_stats(mongo_collection):
    try:
        # mongo collection stats
        collection_stats = mongo_collection.database.command("collstats", mongo_collection.name)
        total_size, total_count, avg_doc_size = round((collection_stats.get("size", 0)/(1024 ** 2)),2) , collection_stats.get("count", 0), collection_stats.get("avgObjSize", 0) 
        logger.info(f"Total size of the collection: {total_size}mb, total count: {total_count}, avg_doc_size: {avg_doc_size}bytes")

        return total_size, total_count, avg_doc_size
    except Exception as e:
        logger.error(f"error in collection stats: {e}",exc_info= True)
